...
if (!Number.isSafeInteger(PORT) || PORT < 1024 || PORT > 65_535) {
  throw new Error("DEUNA_MEDIA_IMPORT_WORKER_PORT debe ser un puerto local válido entre 1024 y 65535.");
}
if (TOKEN.length < 32 || TOKEN.length > 256) {
  throw new Error("DEUNA_MEDIA_IMPORT_WORKER_TOKEN debe tener entre 32 y 256 caracteres.");
}
...
// Allowlist explícita: cubre las redes principales y varios proveedores
// adicionales sin convertir el worker en un proxy web genérico.
const platformHosts = [
...
function textError(response, status, message) {
...
function parseHttpsUrl(value) {
  ...
  if (
    ...
    throw new Error("El video remoto debe usar una URL HTTPS pública sin credenciales ni puertos alternativos.");
  }
  return url;
}
...
function parsePlatformUrl(value) {
  ...
  if (!platformHosts.some((allowedHost) => hostnameMatches(hostname, allowedHost))) {
    throw new Error("La plataforma de ese enlace no está habilitada para importación automática.");
  }
  return url;
}
...
async function resolvePublicAddress(hostname) {
  ...
  if (!addresses.every(({ address, family }) => addressIsPublic(address, family))) {
  ...
  const limiter = new Transform({
    transform(chunk, _encoding, callback) {
      ...
      if (total > maximumBytes) {
        callback(new Error("El video remoto supera el límite permitido."));
        return;
      }
      callback(null, chunk);
    },
    ...
    const request = httpsRequest(
      ...
      (response) => {
        ...
        if (!contentType || !allowedContentTypes.has(contentType)) {
          response.resume();
          reject(new Error("La URL directa devuelve una página web u otro contenido, no un archivo de video compatible."));
          return;
        }
...
async function downloadDirectVideo(value, destinationPath) {
  ...
  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount += 1) {
    ...
    if (response.statusCode >= 300 && response.statusCode < 400 && response.location) {
      ...
      current = parseHttpsUrl(new URL(response.location, current).toString());
      continue;
    }
    return {
...
function classifyYtDlpFailure(stderr) {
  ...
  if (
    normalized.includes("sign in") || normalized.includes("login") ||
    normalized.includes("private") || normalized.includes("age-restricted") ||
    normalized.includes("members-only") || normalized.includes("unavailable") ||
    normalized.includes("cookies")
  ) {
    return new Error("La plataforma exige login/cookies o el video no es público.");
  }
  if (normalized.includes("unsupported url") || normalized.includes("no suitable extractor")) {
    return new Error("yt-dlp no pudo extraer ese enlace. Actualiza yt-dlp o prueba otra URL pública del mismo video.");
  }
  if (normalized.includes("max-filesize") || normalized.includes("file is larger")) {
    return new Error("La copia temporal del video supera 512 MB.");
  }
  return new Error("No se pudo obtener el video desde la plataforma. Comprueba el enlace y actualiza yt-dlp.");
}

function runPlatformYtDlp(sourceUrl, temporaryDirectory) {
  return new Promise((resolve, reject) => {
    ...
    child.stderr?.on("data", (chunk) => {
      if (stderr.length < MAX_ERROR_CHARS) {
        stderr += chunk.slice(0, MAX_ERROR_CHARS - stderr.length);
      }
    });
    ...
    child.once("close", (code, signal) => {
      ...
      clearTimeout(timeout);
      if (signal) {
        reject(new Error("La importación de plataforma excedió el tiempo permitido."));
        return;
      }
      if (code !== 0) {
...
async function downloadPlatformVideo(value, temporaryDirectory) {
  const sourceUrl = parsePlatformUrl(value).toString();
  await runPlatformYtDlp(sourceUrl, temporaryDirectory);
  ...
  if (!stats.isFile() || stats.isSymbolicLink() || stats.size <= 0 || stats.size > MAX_PLATFORM_SOURCE_BYTES) {
    throw new Error("La copia temporal de la plataforma supera el límite de 512 MB o no es válida.");
  }
  return {
    filePath,
    bytes: stats.size,
...
const server = createServer(async (request, response) => {
  ...
  if (request.method !== "POST" || request.url !== "/source") {
    textError(response, 404, "No encontrado.");
    return;
  }
  if (!authorized(request)) {
    textError(response, 401, "No autorizado.");
    return;
  }
  if (activeJob) {
    textError(response, 429, "Ya hay una importación multimedia en curso.");
    return;
  }
  ...
  try {
    ...
    if (payload.kind === "direct") {
      ...
      if (typeof payload.url !== "string") throw new Error("Falta el enlace de plataforma.");
      result = await downloadPlatformVideo(payload.url, temporaryDirectory);
    } else {
    ...
    if (!response.headersSent) {
      textError(response, 400, error instanceof Error ? error.message : "La importación multimedia falló.");
...
server.listen(PORT, HOST, () => {
  console.log(`DeUna media import worker escuchando sólo en ${HOST}:${PORT}.`);
});
