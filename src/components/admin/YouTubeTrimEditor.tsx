...
export default function YouTubeTrimEditor({
      ...
      if (
        ...
        if (!initialApplied.current) {
          ...
          const safeStart = clamp(
            ...
            Math.max(
              0,
              safeDuration - MIN_SELECTION_SECONDS
            )
          );
          const safeEnd = clamp(
            requestedEnd,
  ...
  function markStartAtPlayhead() {
    ...
    const desired = clamp(
      ...
      Math.max(
        0,
        duration - MIN_SELECTION_SECONDS
      )
    );
      ...
      {mediaError ? (
        ...
        <>
          ...
          <p className={styles.help}>
            Arrastra IN y OUT sobre la línea de tiempo. El tramo puede durar hasta 30 segundos. En modo YouTube directo se guarda sólo el ID y este intervalo: no se descarga ni se convierte el video.
          </p>
        </>
      )}
    </div>
