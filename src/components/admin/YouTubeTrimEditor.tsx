...
function formatTime(value: number) {
  ...
  const base = `${String(minutes).padStart(2, "0")}:${seconds
    .toFixed(1)
    .padStart(4, "0")}`;
...
export default function YouTubeTrimEditor({
  ...
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      ...
      if (message.event === "onError") {
        setMediaError(
          "YouTube no permite reproducir este video embebido o el video ya no está disponible."
        );
        setReady(false);
        onTrimChange(null);
        return;
      ...
      if (message.event === "onStateChange") {
        setPlaying(Number(message.info) === 1);
        return;
      }
      ...
      if (
        ...
        if (
          ...
          if (!initialApplied.current) {
            ...
            const requestedStart = initialTrim?.startSeconds ?? 0;
            const requestedEnd =
              initialTrim?.endSeconds ??
              Math.min(
                safeDuration,
                MAX_PREVIEW_DURATION_SECONDS
              );
            const safeStart = clamp(
              requestedStart,
              0,
              Math.max(0, safeDuration - MIN_SELECTION_SECONDS)
            );
            const safeEnd = clamp(
              requestedEnd,
            ...
            setStartSeconds(roundSeconds(safeStart));
            setEndSeconds(roundSeconds(safeEnd));
            setCurrentTime(roundSeconds(safeStart));
            sendCommand("seekTo", [safeStart, true]);
          }
        }
  ...
  }, [initialTrim, loopSelection, onTrimChange, sendCommand, trim]);
  ...
  function markStartAtPlayhead() {
    ...
    if (nextEnd - desired > MAX_PREVIEW_DURATION_SECONDS) {
      nextEnd = Math.min(
        duration,
        desired + MAX_PREVIEW_DURATION_SECONDS
      );
  ...
  function markEndAtPlayhead() {
    ...
    if (desired - nextStart > MAX_PREVIEW_DURATION_SECONDS) {
      nextStart = Math.max(
        0,
        desired - MAX_PREVIEW_DURATION_SECONDS
      );
      ...
      {mediaError ? (
        ...
        <>
          ...
          <p className={styles.help}>
            Arrastra IN y OUT sobre la línea de tiempo. El tramo puede durar hasta 30 segundos. En la web pública no se descarga ni se convierte este video: se guarda sólo el ID y este intervalo.
          </p>
        </>
      )}
    </div>
