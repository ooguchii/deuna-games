...
const widthLimits = { desktop: [1101, 3840], tablet: [681, 1100], mobile: [320, 680] };
...
export default function HomeHeroLivePreview({ games, presentation, device, playing, onSelectPosition, background }: {
  ...
  const [availableWidth, setAvailableWidth] = useState(viewports.desktop.width);
  const [sizes, setSizes] = useState(viewports);
  ...
  return <div ref={container} className={styles.livePreview}>
    <div className={styles.previewToolbar}>
      ...
      <div>
        <label>Ancho de pantalla <input key={`${device}-width`} type="number" min={widthLimits[device][0]} max={widthLimits[device][1]} defaultValue={width} onBlur={(event) => {
        ...
        <label>Alto de pantalla <input key={`${device}-height`} type="number" min={320} max={2160} defaultValue={height} onBlur={(event) => {
          const value = Number(event.target.value);
