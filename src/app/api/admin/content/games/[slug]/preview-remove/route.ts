...
export async function POST(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  ...
  if (!revision.success) return adminRedirect(authorized.adminOrigin, `${target}?estado=solicitud&seccion=multimedia`);
  try {
    ...
    const result = await saveGameMediaDraft(slug, revision.data, authorized.session.userId, { previewClip: undefined });
    ...
    return adminRedirect(authorized.adminOrigin, `${target}?estado=preview-quitado&seccion=multimedia`);
  } catch {
