...
const DUMMY_SALT = Buffer.from(
  "deuna-public-account-dummy-salt-v1",
  "utf8"
);
...
export function validateAccountPassword(password: string) {
  ...
  if (password.length < ACCOUNT_PASSWORD_MIN_LENGTH) {
    issues.push(
      `Debe tener al menos ${ACCOUNT_PASSWORD_MIN_LENGTH} caracteres.`
    );
  }
  ...
  if (
    ...
    issues.push(
      `No puede superar ${ACCOUNT_PASSWORD_MAX_LENGTH} caracteres.`
    );
  }

  if (!/\S/u.test(password)) {
    issues.push("No puede estar formada sólo por espacios.");
  }
...
export async function hashAccountPassword(password: string) {
  const issues = validateAccountPassword(password);
...
export async function consumeDummyAccountPasswordWork(
  password: string
) {
  const bounded = password.slice(0, ACCOUNT_PASSWORD_MAX_LENGTH);
