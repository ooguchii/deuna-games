  ...
  while (cursor < input.length) {
    ...
    if (chunkType === "IHDR") {
      ...
      if (
        ...
        !pngBitDepthIsValid(bitDepth, nextColorType) ||
        compression !== 0 ||
        filter !== 0 ||
...
function processGif(
  ...
  const globalTableSize = globalTable
    ? 3 * (1 << ((packed & 0x07) + 1))
    : 0;
  const dataStart = 13 + globalTableSize;

  if (dataStart > input.length) return null;
  ...
  while (cursor < input.length) {
    ...
    const localTableSize = localTable
      ? 3 * (1 << ((imagePacked & 0x07) + 1))
      : 0;
    const lzwOffset = cursor + 10 + localTableSize;

    if (lzwOffset + 1 > input.length) return null;
