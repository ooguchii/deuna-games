...
# 1) Hardware: remove the obsolete import whose only purpose used to be side effects.
replace_exact(
...
# 2) FPS: eliminate artificial lower floors while keeping mathematically valid non-negative output.
replace_exact(
    "src/features/game-finder/performance-model.ts",
    "  fps = Math.max(8, fps);",
    "  fps = Math.max(0, fps);",
)
replace_exact(
...
# 3) Windows: unknown means unknown, never Windows 11 by default.
replace_exact(
...
# 4) Updates: neutral copy and consistent navigation for entries without a download source.
replace_exact(
    "src/components/updates/UpdatesCatalogClient.tsx",
...
# 5) Remove trivial UniversalGameCard wrappers and use the canonical component directly.
wrapper_replacements = {
    "src/components/home/PopularGames.tsx": [
        (
    ...
    "src/components/home/RecommendedGames.tsx": [
        (
            'import CardCarousel from "@/components/ui/CardCarousel";\n',
            'import CardCarousel from "@/components/ui/CardCarousel";\nimport UniversalGameCard from "@/components/ui/UniversalGameCard";\n',
        ),
    ...
    "src/components/home/RecentlyAdded.tsx": [
        (
            'import CardCarousel from "@/components/ui/CardCarousel";\n',
            'import CardCarousel from "@/components/ui/CardCarousel";\nimport UniversalGameCard from "@/components/ui/UniversalGameCard";\n',
        ),
    ...
    "src/components/home/GamesForYourPC.tsx": [
        (
            'import CardCarousel from "@/components/ui/CardCarousel";\n',
            'import CardCarousel from "@/components/ui/CardCarousel";\nimport UniversalGameCard from "@/components/ui/UniversalGameCard";\n',
        ),
...
for path, pairs in wrapper_replacements.items():
    for old, new in pairs:
        replace_exact(path, old, new)

for path in [
    "src/components/home/GameCard.tsx",
    "src/components/home/LowSpecGameCard.tsx",
    "src/components/home/RecentGameCard.tsx",
]:
    Path(path).unlink()

# 6) Mobile menu accessibility: semantic button, exposed state, Escape, focus and scroll management.
replace_exact(
...
# 7) build:secure portability: neutral temporary directory and direct npm executable.
replace_exact(
...
# 8) Neutral Spanish cleanup in public/source/tool copy. The checker itself is handled explicitly below.
...
skip_neutral_files = {
    Path("tools/check-public-privacy.mjs"),
    Path("tools/audit-finish.py"),
}

for root in [Path("src"), Path("tools")]:
    for file in root.rglob("*"):
        if file in skip_neutral_files:
            continue
...
# 9) Harden the neutral-language checker across tools as well as src/public/ops/root files.
replace_exact(
...
# 10) Docker: there is no Dockerfile/compose deployment, so the ignore file is orphaned.
