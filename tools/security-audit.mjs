...
const skipNpmAudit = process.argv.includes(
  "--skip-npm-audit"
);
...
const criticalPatterns = [
  ...
  {
    name: "URL de base de datos con password",
    pattern:
...
function addTrackedFileChecks(trackedFiles) {
  for (const tracked of trackedFiles) {
    ...
    if (
      ...
      errors.push(`Archivo .env trackeado por Git: ${tracked}`);
    }
    ...
    if (
      ...
      errors.push(`Reporte local trackeado por Git: ${tracked}`);
    }
    ...
    if (normalized === "src/theme/palette.ts") {
      errors.push(
    ...
    if (/(^|\/)\.(idea|vscode)\//.test(normalized)) {
      warnings.push(`Configuración de IDE trackeada: ${tracked}`);
    }

    if (/\.(pem|key|p12|pfx|kdbx|crt|cer)$/i.test(normalized)) {
      errors.push(`Archivo criptográfico/credencial trackeado: ${tracked}`);
    }

    if (
      /(?:\.backup|\.dump|\.sql\.gz)$/i.test(normalized) ||
      /(^|\/)pg_dump[^/]*$/i.test(normalized)
    ) {
      errors.push(`Copia de PostgreSQL trackeada por Git: ${tracked}`);
    }
  }
}

async function scanPublicLeaks(publicFiles) {
  for (const file of publicFiles) {
    ...
    if (
      ...
      errors.push(`Archivo que no debe estar en public/: ${relative(file)}`);
    }
  }
}
...
async function scanTrackedTextFiles(trackedFiles) {
  const candidates = [
    ...new Set(
      trackedFiles
        .map((tracked) => path.join(root, tracked))
    ...
    try {
      ...
      for (const entry of criticalPatterns) {
        ...
        if (entry.pattern.test(content)) {
          errors.push(`${entry.name} detectado en ${fileName}`);
        }
      }

      genericSecretPattern.lastIndex = 0;
      if (genericSecretPattern.test(content)) {
        errors.push(`Posible secreto hardcodeado en ${fileName}`);
      }
      ...
      warnings.push(`No se pudo analizar ${relative(file)}`);
    }
  }
}

async function scanImages(publicFiles) {
  for (const file of publicFiles) {
    const extension = path.extname(file).toLowerCase();
    ...
    try {
      ...
      if (
        /[A-Za-z]:\\Users\\/.test(ascii) ||
        /\/Users\//.test(ascii) ||
        /\/home\//.test(ascii)
      ) {
        errors.push(`Ruta local embebida dentro de imagen: ${fileName}`);
      }

      if (/EXIF/i.test(ascii) || /xmpmeta/i.test(ascii)) {
        warnings.push(`La imagen podría contener EXIF/XMP: ${fileName}`);
      }
    } catch {
      warnings.push(`No se pudo inspeccionar binario: ${relative(file)}`);
    }
  }
}

function checkGitIdentity() {
  const name = readGitLines(["config", "user.name"])[0];
  const email = readGitLines(["config", "user.email"])[0];
...
function runNpmAudit() {
  ...
  if (result.error) {
    errors.push("No se pudo ejecutar npm audit.");
    return;
  }
...
console.log("");
console.log("==============================================");
console.log(" DeUna Games - auditoría de seguridad");
console.log("==============================================");
console.log("");
...
if (trackedFiles.length === 0) {
  ...
  addTrackedFileChecks(trackedFiles);
  await scanTrackedTextFiles(trackedFiles);
}
...
if (errors.length > 0) {
  console.error("AUDITORÍA: NO APROBADA");
  console.error("Corrige los ERROR antes de publicar.");
  process.exit(1);
}

console.log("AUDITORÍA: APROBADA");
console.log(
  "No se encontraron bloqueadores en archivos versionados ni en el contenido público."
);
